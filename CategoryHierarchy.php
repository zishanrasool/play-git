<?php
//https://github.com/amochohan/CategoryHierarchy/blob/master/app/Helpers/CategoryHierarchy.php
/**
...
class CategoryHierarchy {
    ...
    private function htmlFromArray($hierarchicalArray, $addCheckboxes = false, $listClass = 'list-unstyled') {
        ...
        foreach($hierarchicalArray as $item) {
            ...
            $html .= "<li>".($addCheckboxes ? '<input type="checkbox" name="gdfg[]" value="'.$item['id'].'" '.(isset($item['checked']) && ($item['checked']=true) ? 'checked' : '').'> ' : '') . $item['name']."</li>";
